...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as ResumePulseEngineRequest;
    ...
    const result = await resumePulseEngine({ engineId });
    ...
    const message =
      error instanceof Error ? error.message : "Could not resume Pulse engine.";

    console.error("[admin/pulse/resume]", message);

    return NextResponse.json(
      { ok: false, error: "Could not resume Pulse engine." },
      { status: 500 },
    );
  }
}
