-- Phase 6A: user-based staking and settlement alongside legacy anonymous positions.

create or replace function stake_on_market_for_user(
  p_market_id uuid,
  p_user_id uuid,
  p_side market_side,
...
begin
  ...
  values (p_market_id, p_user_id, p_side, p_stake_credits)
  ...
  values (p_user_id, p_market_id, -p_stake_credits, 'stake debit');
  ...
  loop
    if v_position.side = p_outcome then
      ...
      if found then
        ...
        elsif v_position.anonymous_session_id is not null then
          update cope_credit_accounts
          set
            balance_credits = balance_credits + v_payout,
            total_won_credits = total_won_credits + v_payout,
